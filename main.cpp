...
// ЗАВДАННЯ 1: Array32 
// Знайти номер першого локального мінімуму (менший за сусідів)
void task_array_32() {
    int arr[N];
    int n;
    input_array(arr, n);
    ...
    int result_index = 0; // Якщо не знайдемо, залишиться 0
    
    for (int i = 0; i < n; i++) {
        bool is_min = true;
        
        // Перевірка лівого сусіда
        if (i > 0 && arr[i] >= arr[i-1]) is_min = false;
        // Перевірка правого сусіда
        if (i < n - 1 && arr[i] >= arr[i+1]) is_min = false;
        
        if (is_min) {
            result_index = i + 1; // Порядковий номер (з 1)
            break; // Потрібен перший знайдений
        }
    }
    
    cout << "Номер першого локального мінімуму: " << result_index << endl;
}
